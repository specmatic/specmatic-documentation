...
Click on `Spec` tab, restore `petstore.yaml` back to its original state by pasting <a href="pathname://original_petstore_spec.html" target="_blank">this content</a> back in, save it, and run the tests. With the example back, the tests will pass once again.
...
Restore the original content of the spec back, using the Spec tab. You can copy the original spec from [here](pathname://original_petstore_spec.html), paste it into the `Spec` tab, and save it.
