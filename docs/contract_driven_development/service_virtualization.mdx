...
======================
<img alt="Mocking" src="/img/mocking.png" />
...
- Make sure you have installed Specmatic from [Download page](/docs/download).
...
You can enable strict mode using configuration as described [here](/docs/references/configuration/stub-configuration#strict-mode).
...
Refer to [Dictionary with service virtualization](/docs/features/dictionary#dictionary-with-service-virtualization) for more details.
...
This utility can now be used in your test ```setup``` / ```beforeAll``` method to start the stub server. Specmatic automatically looks for your [Specmatic configuration](/docs/references/configuration) file in project root directory / classpath to locate your API Specification files that need to run as part of the stub server.
...
> **Note:** If you're looking to modify headers during test using Specmatic's `test` command, then please refer to test-specific header modifications documentation : [Using Hooks during Tests](/docs/contract_driven_development/contract_testing#hooks)
