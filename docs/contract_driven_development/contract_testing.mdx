...
However, the response example named `FETCH_EMPLOYEE_SUCCESS` is verified and used in [service virtualization](/docs/contract_driven_development/service_virtualization#inline-examples).
...
The complete test data format can be referred to [here](/docs/features/external_examples#example-format).
...
If the OpenAPI contract defines API authentication using security schemas, these information will be used by Specmatic when running contract tests. Read more about it on the page on [authentication](/docs/features/authentication).
...
Also, please note that this use of `[WIP]` differs from the WIP tags given [here](/docs/contract_driven_development/backward_compatibility#handling-changes-in-progress). WIP tags can be applied to the path/endpoint, while this works only on example names.
...
More details on overlays can be found in the [Overlays Documentation](/docs/features/overlays).
...
> **Note:** If you're looking to modify headers during service virtualization using Specmatic's `stub` command, then please refer to stub-specific header modifications documentation : [Using Hooks during Stub Creation](/docs/contract_driven_development/service_virtualization#hooks)
...
More details [here](/docs/contract_driven_development/service_virtualization#using-matching-branches-in-the-central-contract-repo).
