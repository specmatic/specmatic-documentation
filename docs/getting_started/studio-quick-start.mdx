...
Click on `Spec` tab, restore `petstore.yaml` back to its original state by pasting [this content](/docs/hidden/original_petstore_spec.mdx) back in, save it, and run the tests. With the example back, the tests will pass once again.
...
Restore the original content of the spec back, using the Spec tab. You can copy the original spec from [here](/docs/hidden/original_petstore_spec.mdx), paste it into the `Spec` tab, and save it.
...
Please restore `petstore.yaml` to its [original state](/docs/hidden/original_petstore_spec.mdx) before proceeding with this section.
...
[**Learn more about Mocking / Smart Mocks here.**](/docs/contract_driven_development/service_virtualization)
