...
`baseurls` - needed when running contracts as test as part of [authentication](/docs/Features/authentication)

### Variables

`variables` - these values are plugged into the Examples rows of an auth contract for [authentication](/docs/Features/authentication), or even when running regular contract tests
