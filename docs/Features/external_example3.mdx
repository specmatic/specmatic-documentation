...
## Identifying Examples with Competing Requests <span className="badge badge--primary" style={{ fontSize: '0.55rem' }}> Commercial </span>
...
We will continue to use the `employee_details.yaml` [spec from above](/features/external_examples#creating-validating-and-fixing-examples).
...
Similar to [Competing Requests by Identical Values](/features/external_examples#competing-requests-by-identical-values), lets see how [data type-based examples](/contract_driven_development/service_virtualization#data-type-based-examples) could end up competing for some incoming requests.
...
Now, unlike [Competing Requests by Identical Data Type Values](/features/external_examples#competing-requests-by-identical-data-type-values)
